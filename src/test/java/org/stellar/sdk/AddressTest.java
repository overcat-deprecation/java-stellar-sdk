...
public class AddressTest {
  ...
  public void testConstructorAccountId() {
    ...
    assertEquals(address.getAddress(), accountId);
  ...
  public void testConstructorContractId() {
    ...
    assertEquals(address.getAddress(), contractId);
  ...
  @Test
  public void testFromAccountString() {
    String accountId = "GA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVSGZ";
    Address address = Address.fromAccount(accountId);
    assertEquals(address.getAddress(), accountId);
    assertEquals(address.getType(), Address.AddressType.ACCOUNT);
  }
  ...
  @Test
  public void testFromAccountInvalidAddressThrows() {
    String accountId = "GINVALID";
    try {
      Address.fromAccount(accountId);
      fail();
    } catch (IllegalArgumentException e) {
      Assert.assertEquals("accountId is invalid", e.getMessage());
    }
  }

  @Test
  public void testFromContractString() {
    String contractId = "CA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUWDA";
    Address address = Address.fromContract(contractId);
    assertEquals(address.getAddress(), contractId);
    assertEquals(address.getType(), Address.AddressType.CONTRACT);
  }
  ...
  @Test
  public void testFromContractInvalidAddressThrows() {
    String contractId = "CINVALID";
    try {
      Address.fromContract(contractId);
      fail();
    } catch (IllegalArgumentException e) {
      Assert.assertEquals("contractId is invalid", e.getMessage());
    }
  }
  ...
  public void testToSCAddressContract() throws IOException {
    ...
    Address address = Address.fromContract(contract);
    SCAddress scAddress = address.toSCAddress();
  ...
  public void testFromSCAddressContract() throws IOException {
    ...
    Address address = Address.fromSCAddress(scAddress);
    String contract = "CA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUWDA";
    assertEquals(address.getAddress(), contract);
    assertEquals(address.getType(), Address.AddressType.CONTRACT);
  }
